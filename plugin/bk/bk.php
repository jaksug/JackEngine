...
class Bk extends XP_Controller
{
    ...
    {
        set_page_title('Bimbingan Konseling SMKN 1 Gunungputri');
        ...
        $news_cache_id = 'berita_bk_'.$page_news;
        
        if($this->cache->valid($news_cache_id)){
            $data = $this->cache->get($news_cache_id);
        } else {
            $offset     = ($page - 1 ) * $per_page;
            $sql_query  = "SELECT a.id,a.url,a.isi,a.time,a.judul,b.nama,b.xp_id,b.xp_nama FROM berita a,xpreser b,category c WHERE ";
            if(!empty($category)) {
            ...
            $sql_query .= " AND status='published' AND kategori_id=2   ORDER BY a.id DESC LIMIT $offset,$per_page";
        ...
        $jumlah     = ceil($count / $per_page);
        $page_info  = array('page'=>$page,'jumlah'=>$jumlah,'berita'=>$count);
    ...
    {  
        ...
        $post_id    = $data[0]['id'];
        $komentar   = $this->database->rawSelect("SELECT a.comment,b.nama,b.aura,b.xp_nama FROM comment a,xpreser b where a.eks_id='$post_id' AND b.xp_id=a.xp_id");
    ...
    function add_comment()
    {
        ...
        if(!$this->user->status) {
            ...
            $db         = getDB();
            $comment_id = rand();
            $data       = array('id'=>'','xp_id'=>($this->user->id) ? $this->user->id:'pengunjung','eks_id'=>$post_id,'comment'=>$koment,'nama'=>$komentator);
