<?php if(!defined('XP-ENGINE')) exit('Anda tidak berhak atas ini');
...
if ( ! function_exists('form_open'))
{
	function form_open($action = '', $attributes = '', $hidden = array())
	{
		...
		$action = ( strpos($action, '://') === FALSE) ? $action : $action;
		...
		$hidden[$xp->keamanan->csrf_token_name] = $xp->keamanan->csrf_hash;
	...
	function _attributes_to_string($attributes, $formtag = FALSE)
	{
	   $xp = getInstance();
		if (is_string($attributes) AND strlen($attributes) > 0)
		{
			...
			{
				$attributes .= ' accept-charset="'.$xp->setting['charset'].'"';
			}
		...
		if (is_array($attributes) AND count($attributes) > 0)
		{
			...
			{
				$atts .= ' accept-charset="'.strtolower($xp->setting['charset']).'"';
			}
