...
class XP_Image {
    ...
    function cek($file)
   	{ 
   	    $this->file = $file;
       	$this->ekstensi = strrchr($this->file, '.');
       	$this->ekstensi = strtolower($this->ekstensi);
       	$this->filename=$this->_openImage($this->file);
	...
	function _convert()
	{
		switch($this->ekstensi) {
            case '.jpg':
            case '.jpeg':
                    imagegif($this->filename, $this->file);
                    imagejpeg($this->filename, $this->file);
      		...
      		case '.gif':
                    ...
                    break;
      		case '.png':
      		        imagejpeg($this->filename, $this->_file);
        			imagepng($this->filename, $this->file);
                    break;
            default:
          		    die("Bir Terslik Var");
		}
    ...
    function potong($nw, $nh, $source, $stype, $dest) {
